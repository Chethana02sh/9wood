...
//		Reporter.log("Campaign is edited",true);
		...
		String NewCampName = eLib.readDataFromExcel("Campaign", 1, 0)+randNum1;
		
		addCampaignPage.campaignname(NewCampName);
		addCampaignPage.getSaveBtn().click();
		
		String EditedCampName = driver.findElement(By.xpath("//span[.='"+NewCampName+"']")).getText();
		Assert.assertEquals(NewCampName, EditedCampName);
		
		Reporter.log("campaign is edited successfully",true);
